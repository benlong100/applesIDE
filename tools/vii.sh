...
aesc() { printf '%s' "$1" | sed -e 's/\\/\\\\/g' -e 's/"/\\"/g'; }
...
#
# The argument is interpolated into an AppleScript string literal, so a
# double quote in it ends that literal early and the rest of the line is
# parsed as AppleScript. Inherited from ZipEdit, where it almost never showed:
# prose rarely contains a quote and the suite never typed one. Applesoft is
# made of them -- PRINT "HELLO" -- and the failure is silent and confusing,
# because osascript reports nothing useful and the text simply never arrives.
# Backslashes have the same problem and are escaped first, or escaping the
# quotes would double their own backslashes.
text) as "tell (last machine) to type text \"$(aesc "$1")\"" ;;
...
line) as "tell (last machine) to type line \"$(aesc "$1")\"" ;;
